...
3. **Pre-release benchmark and training-utility evidence.** The public preview reports two kinds of evidence: benchmark-construction signals from SOTA-solver evaluation and Qwen-family scale-consistency checks; and an early downstream training signal where Qwen3-4B-Instruct variants trained with around `2,000` AgenQA examples transfer positively to AIM24, HMMT-FEB, and SciBench while keeping GPQA / GPQA-Diamond essentially unchanged.
