...
图~\ref{fig:candidate-prefix-control} 用一个构造的三步数学例子把这些抽象落到 candidate-prefix control event。Panel A 展示 Premises、Intermediate Memory 与 accepted chain 如何逐步增长；Panel B 展示错误 candidate answer 如何通过 solver feedback 被局部 revise；Panel C 展示同一 candidate prefix 的 Step View 与 Folded View。这里的 Step View 对应局部 Edge evaluation，Folded View 对应隐藏中间结果的 Path-Fold projection。该例子仅用于解释机制，不是 actual AgenQA run、gold case、benchmark result 或 difficulty evidence。

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/figure2_candidate_prefix_dual_view.png}
  \caption{A worked illustration of premise-grounded chain growth, local revision, and dual-view evaluation of a candidate prefix. This constructed example is not an actual AgenQA run.}
  \label{fig:candidate-prefix-control}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.94\linewidth]{figures/figure3_agentic_operators_chain_state.png}
