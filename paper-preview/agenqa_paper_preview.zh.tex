...
\begin{enumerate}
  \item \textbf{Edge/Path-grounded Chain-of-KQA formalism.} AgenQA 将 challenging reasoning-QA synthesis 表示为一条逐步增长的 step-verifiable dependency chain。每个局部 transition 都带有可检查的 known state、answer-equivalent key fact 和 dependency certificate；同一条 chain 再被投影为 Edge View 与 Path View，从而把 step-level correctness control 和 global difficulty amplification 明确拆开。
  \item \textbf{Agentic state-transition harness for synthesis and repair.} AgenQA 不把 agent 当成一次性出题器，而是把 Director、Operators 和 Evaluator 组织成显式 reasoning state 上的闭环。Extension、revision、folding、solver feedback、consensus 和 acceptance signals 都作用在同一 Chain-of-KQA object 上，使 generation、verification 和 repair 成为一个可追踪生命周期。
  \item \textbf{Benchmark-construction validation with model-discrimination signals.} 当前 public preview 以 benchmark construction 为主验证面：Edge-side solver signals 用作 correctness 与 well-posedness gates，Path-side solver distributions 用作 difficulty 与 discrimination signals。阶段性结果显示，Path View items 能区分 SOTA solvers 与不同规模 Qwen-family models，并保留可解释的难度来源。
\end{enumerate}
...
本节保留两张最能说明问题的表：一张展示强模型在 Path View 题目上的表现，另一张展示 Qwen-family 内 benchmark accuracy 与模型规模/理论能力的正相关关系。两张表共同说明：AgenQA 的 Path-Fold 不只是产生更长题面，而是在保留生成侧 dependency state 的同时，为 solver-facing benchmark 构造出可测的 difficulty 与 model-discrimination signal。
