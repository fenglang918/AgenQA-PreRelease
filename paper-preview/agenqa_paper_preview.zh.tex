...
本节保留两张最能说明问题的表：一张展示强模型在 Path View 题目上的表现，用于观察强模型区间的难度和区分区域；另一张展示 Qwen-family 内 benchmark accuracy 与模型规模/理论能力的正相关关系，用于做 benchmark 合理性的 scale-consistency sanity check。两张表共同说明：AgenQA 的 Path-Fold 不只是产生更长题面，而是在保留生成侧 dependency state 的同时，为 solver-facing benchmark 构造出可测的难度、强模型区分信号和模型尺寸一致性信号。
