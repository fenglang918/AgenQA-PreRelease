...
3. **Pre-release benchmark and training-utility evidence.** 当前 public preview 报告两类 evidence：benchmark-construction signals 包括 SOTA solver 评测和 Qwen-family scale-consistency sanity check；early downstream training signal 显示，约 `2,000` 条 AgenQA 数据训练后的 Qwen3-4B-Instruct variants 在 AIM24、HMMT-FEB 和 SciBench 上出现正向迁移，同时 GPQA / GPQA-Diamond 基本持平。
